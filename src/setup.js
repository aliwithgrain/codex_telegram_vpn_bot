...
async function askRequiredQuestion(rl, question, hintDefault) {
  while (true) {
    ...
    const value = raw.trim() || hintDefault;
...
async function main() {
  ...
  answers.RUNTIME_CONFIG_PATH = existing.RUNTIME_CONFIG_PATH || 'config/runtime.config.json';

  const lines = [
    '# Generated by setup wizard',
    '# Do not commit this file',
    ...Object.entries({ ...existing, ...answers }).map(([key, value]) => `${key}=${value}`)
  ];
